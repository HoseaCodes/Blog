...
The client half of the API. Every resource family has a hook-shaped module in [`src/API/`](https://github.com/HoseaCodes/Blog/tree/master/src/API) that wraps axios calls and owns its own state; they are composed into `GlobalState` and consumed with `useContext`.
