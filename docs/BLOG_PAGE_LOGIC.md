...
The `/blog` page renders an HBR-style landing layout: a most-liked hero, a three-column featured grid, and a right-rail "The Latest" list. The picks are derived from a single `GET /api/articles` fetch in a `useEffect` in [`src/Pages/Articles/Articles.jsx`](../src/Pages/Articles/Articles.jsx) — search for `fetchMostLikedArticle`, around line 1317.
...
- **Schema** — [`models/article.js`](../models/article.js): `createdAt`, `likes`, `categories`, `draft`, `archived`.
