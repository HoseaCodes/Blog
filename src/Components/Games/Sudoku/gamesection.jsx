...
export const GameSection = (props) => {
  ...
  function _selectedCell(indexOfArray, value, highlight) {
    if (value !== '0') {
      if (initArray[indexOfArray] === '0') {
        return (
          <td className={`sudoku-game__cell sudoku-game__cell--userfilled sudoku-game__cell--${highlight}selected`} key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
        )
      } else {
        return (
          <td className={`sudoku-game__cell sudoku-game__cell--filled sudoku-game__cell--${highlight}selected`} key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
        )
      }
    } else {
      return (
        <td className={`sudoku-game__cell sudoku-game__cell--${highlight}selected`} key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
      )
    }
  }
  ...
  function _unselectedCell(indexOfArray, value) {
    if (value !== '0') {
      if (initArray[indexOfArray] === '0') {
        return (
          <td className="sudoku-game__cell sudoku-game__cell--userfilled" key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
        )
      } else {
        return (
          <td className="sudoku-game__cell sudoku-game__cell--filled" key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
        )
      }
    } else {
      return (
        <td className="sudoku-game__cell" key={indexOfArray} onClick={() => props.onClick(indexOfArray)}>{value}</td>
      )
    }
  }
  ...
  return (
    <section className="sudoku-game">
      <table className="sudoku-game__board">
        <tbody>
          {
            rows.map((row) => {
              return (
                <tr className="sudoku-game__row" key={row}>
                  {
                    rows.map((column) => {
