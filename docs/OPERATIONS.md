...
Then in Chrome DevTools: throttle to low-end mobile, open the Profiler, enable **"Record why each component rendered while profiling"**, and look at the orange-and-darker commits. Work out whether it is a state change, a prop change, or a context change driving the re-render before changing anything — the same observe-first rule as [`CLAUDE.md`](https://github.com/HoseaCodes/Blog/blob/master/CLAUDE.md).
