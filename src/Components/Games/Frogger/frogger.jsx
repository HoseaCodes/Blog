...
function ClickLots() {
  const squeek =
    "https://d2nrcsymqn25pk.cloudfront.net/Assets/Sounds/squeek.wav";
  const win =
    "https://d2nrcsymqn25pk.cloudfront.net/Assets/Sounds/99636__tomlija__small-crowd-yelling-yeah.wav";
  const crashSound =
    "https://d2nrcsymqn25pk.cloudfront.net/Assets/Sounds/crash.wav";
  document.body.style.backgroundColor = "#88cc00";
  const trafficArray = [ambulance, bicycle, bus, cab, car, truck];
  const [vehicle, setVehicle] = useState(
    trafficArray[Math.floor(Math.random() * trafficArray.length)]
  );
  ...
  return (
    <div onClick={() => handleClick()} className="clickWrapper">
      {showMouse && (
        <p className="instructionText">click lots. don't get hit by traffic</p>
      )}
      ...
      <div className="road">
        <img
          className="vehicle"
          style={{ top: `${vehiclePosition}vh` }}
          src={vehicle}
          alt=""
        />
        <img
          className="vehicle2"
          style={{ bottom: `${vehicle2Position}vh` }}
          src={vehicle2}
          alt=""
        />
